...
import type { ColorValueChangeDetail, PrimitiveColorSaveDetail } from '../lib/types.js'
...
export class PrimitiveColorDialog extends LitElement {
  ...
  _submit(event: SubmitEvent) {
    ...
    this.dispatchEvent(
      new CustomEvent<PrimitiveColorSaveDetail>('save', {
      ...
      >
        <form method="dialog" @submit=${this._submit}>
          ...
          <tkn-color-input
            ...
            @value-change=${(event: CustomEvent<ColorValueChangeDetail>) => {
