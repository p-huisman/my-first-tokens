    ...
    return html`
      ...
      >
        <form @submit=${this._submit}>
          <header>
            <div>
              <p class="eyebrow">Primitive gradient</p>
              <h2>${this.initial ? 'Edit gradient' : 'Add gradient'}</h2>
            </div>
          ...
          <tkn-color-input
            ...
            @value-change=${(event: CustomEvent<{ value: string }>) => {
              this.startColor = event.detail.value
              this.error = ''
            }}
          ></tkn-color-input>
          <tkn-color-input
            ...
            .value=${this.endColor}
            @value-change=${(event: CustomEvent<{ value: string }>) => {
              this.endColor = event.detail.value
              this.error = ''
            }}
          ></tkn-color-input>
          <label
            >Angle<input
              ...
              .value=${this.angle}
              @input=${(event: Event) => {
                this.angle = (event.target as HTMLInputElement).value
              }}
          /></label>
          ${this.error ? html`<p class="error" role="alert">${this.error}</p>` : ''}
          <footer>
            <button class="secondary" type="button" @click=${this._close}>Cancel</button
            ><button class="primary" type="submit">${this.initial ? 'Save gradient' : 'Add gradient'}</button>
          </footer>
        </form>
      </dialog>
    `
